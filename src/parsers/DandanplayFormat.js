 ...
 * @license MIT License
 * Parse DandanPlay's JSON Comment data into a Generic Comment List
 **/
function DandanplayParser(jsonDoc){	
    if (jsonDoc !== null) {
        var elems = jsonDoc.Comments;
    } else {
        return null;
    }
	...
	for(var i=0;i < elems.length;i++){
		if(elems[i] != null){
			var text = elems[i].Message;
            var obj = {};
            obj.stime = Math.round(parseFloat(elems[i].Time)*1000);
            obj.size = 25;
            obj.color = parseInt(elems[i].Color);
            obj.mode = parseInt(elems[i].Mode);
            obj.date = parseInt(elems[i].Timestamp);
            obj.pool = parseInt(elems[i].Pool);
            obj.position = "absolute";
            obj.dbid = parseInt(elems[i].CId);
            obj.hash = elems[i].UId + "";
            obj.border = false;
			if(obj.mode < 7){
